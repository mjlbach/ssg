...
class PlaceReward(BaseRewardFunction):
    ...
    def __init__(self, config):
        ...
        self.success_reward_scale = self.config["success_reward_scaling"]
    ...
    def get_reward(self, task, env):
        ...
        for obj in task.books:
            num_goals_satisfied += int(
                obj.states[igibson.object_states.Inside].get_value(task.target)
            )

        reward = self.success_reward_scale * num_goals_satisfied
        temp_reward = reward
...
class PickReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env):
        ...
        for source in task.sources:
            for obj in task.books:
                num_goals_satisfied += int(
                    obj.states[igibson.object_states.Inside].get_value(source)
                )
        count = len(task.books)
